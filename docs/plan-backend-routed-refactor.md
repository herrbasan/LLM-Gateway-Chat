...
- **PD — cutover:** old client retired/read-only, dead browser code removed (client-sdk, mcp-client, conversation state machine), arena re-based as a runner variant, TTS + `/v1/models` proxied. **Arena part DONE 2026-08-26** (landed e06f3dd, fix cluster 5bcd8e2, handover: [_Archive/handover-2026-08-26-arena-fix.md](_Archive/handover-2026-08-26-arena-fix.md)): conversation runs server-side in `server/arena-runner.js`, `chat-arena/` is a spectator view, E2E-verified. Remnants: summary generation + legacy import still direct-gateway; dead `Participant`/`Arena` classes await deletion.
