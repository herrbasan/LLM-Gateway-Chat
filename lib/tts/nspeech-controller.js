...
// The endpoint is server-configured (serverDefaults.endpoint) — never persisted client-side.

import { SpeechPlayer } from './nspeech-client.js';

const DEFAULT_ENDPOINT = '/api/tts'; // same-origin backend proxy → nSpeech
...
// Catalog self-heal (issue #31, TTS half): a failed loadVoices retries on a
// fixed interval forever — same reasoning as the MCP pool's fixed 5s
// reconnect (localhost/LAN: a retry against a down service is an
// instantly-refused connect; no backoff problem to solve). Log transitions
// (down once, recovered once), never attempts.
const VOICES_RETRY_MS = 30000;
// Min spacing between user/player-triggered refreshes (speak failure, player
// error) so a failing engine can't cause a fetch stampede.
const VOICES_REFRESH_MIN_MS = 10000;
...
// Model prefs — per-slot cloud sub-model slug (e.g. minimax_speech_2_8_hd).
// Always a concrete slug: the catalog's default entry is seeded on first
// selection and whenever the stored slug is no longer addressable. The slug
// is sent as the `model` field — nSpeech routes it to that provider model.
const PREF_MODEL = 'tts-model';
const PREF_MODEL_A = 'arena-tts-model-a';
const PREF_MODEL_B = 'arena-tts-model-b';
...
export class NSpeechController {
     ...
     *   - modelSelect:     nui-select (voiceCount=1) — cloud sub-model (hidden for local engines)
     ...
     *   - modelASelect:    nui-select (voiceCount=2) — cloud sub-model A
     *   - modelBSelect:    nui-select (voiceCount=2) — cloud sub-model B
    ...
    constructor({ voiceCount = 1, defaultModel = DEFAULT_MODEL, prefKeyVoice, prefKeySpeed, storage, elements, serverDefaults = {} }) {
        ...
        // State
        this.endpoint = serverDefaults.endpoint || '';
        ...
        this.modelsByEngine = new Map();   // engine name → [{ id, label, default }] (cloud, >1 entry = selectable)
        // Engine selection per slot. 'nspeech' = local dashboard-selected.
        this.engineA = DEFAULT_MODEL;
        this.engineB = DEFAULT_MODEL;
        // Cloud sub-model slug per slot. Seeded with the catalog's default
        // entry per engine — never empty once a cloud engine is selected.
        this.modelA = '';
        this.modelB = '';
        ...
        // Catalog self-heal state (issue #31): retry timer while nSpeech is
        // unreachable, transition marker (log down/recovered once), and the
        // timestamp of the last load attempt (debounce for speak/player-error
        // triggered refreshes).
        this._voicesRetryTimer = null;
        this._voicesWasDown = false;
        this._voicesLastAttempt = 0;
    }
    ...
    async init() {
        if (this.voiceCount === 2) {
            ...
            this.modelA = await this.storage.getPref(PREF_MODEL_A) || '';
            this.modelB = await this.storage.getPref(PREF_MODEL_B) || '';
        } else {
            ...
            this.modelA = await this.storage.getPref(PREF_MODEL) || '';
        }
    ...
    _wireEvents() {
        ...
        // Model selects — cloud sub-model slug (seeded with provider default)
        const onModel = (slot) => (value) => {
            this._setSlotModel(slot, value || '');
            this._onSettingsChanged();
        };
        for (const [el, slot] of [
            [this.elements.modelSelect, 'A'],
            [this.elements.modelASelect, 'A'],
            [this.elements.modelBSelect, 'B'],
        ]) {
            el?.addEventListener('nui-change', (e) => onModel(slot)(this._readSelectValue(e)));
            el?.querySelector('select')?.addEventListener('change', (e) => onModel(slot)(e.target.value));
        }
    ...
    // AND discard its buffered bytes so the next speak() regenerates from the
    // new settings. No-op when idle. Shared by engine/voice/speed/clean/stitch.
    _onSettingsChanged() {
        this.stop();
    }
    ...
    async loadVoices() {
        ...
        this._voicesLastAttempt = Date.now();
        const timer = setTimeout(() => abort.abort(), ENGINE_FETCH_TIMEOUT_MS + VOICE_FETCH_TIMEOUT_MS);

        try {
            // Fetch engine catalog + local voices + model catalog in parallel
            const [enginesResp, localVoicesResp, modelsResp] = await Promise.all([
                ...
                fetch(`${this.endpoint}/v1/models`, { signal: abort.signal }).catch(() => null),
            ]);
            ...
            // Model catalog (cloud sub-models). Entries carry
            // { id (slug), engine, provider_model, label, default }.
            this.modelsByEngine = new Map();
            if (modelsResp && modelsResp.ok) {
                const data = await modelsResp.json();
                for (const m of (Array.isArray(data.data) ? data.data : [])) {
                    if (!m.engine || !m.id || m.id === DEFAULT_MODEL) continue;
                    if (!this.modelsByEngine.has(m.engine)) this.modelsByEngine.set(m.engine, []);
                    this.modelsByEngine.get(m.engine).push({ id: m.id, label: m.label || m.id, default: !!m.default });
                }
            }
            ...
            // Resident local engines (gpu:false, e.g. kokoro) are callable
            // WITHOUT switching — like cloud providers. Their voices get
            // their own engine entry in the selects.
            const residentEngines = engines.filter(e =>
                !e.type && !e.gpu && e.venv_exists && e.name !== this._currentEngine);

            // Fetch cloud + resident engine voices in parallel. A failed fetch
            // logs and yields an empty list — the engine then speaks with no
            // voice rather than a guessed one (see resolveVoice).
            const remoteEngines = [...residentEngines, ...engines.filter(e => e.type === 'cloud')];
            const cloudResults = await Promise.all(remoteEngines.map(async eng => {
                try {
                    const resp = await fetch(`${this.endpoint}/v1/voices?engine=${encodeURIComponent(eng.name)}`, {
                        signal: abort.signal,
                    });
                    if (!resp.ok) {
                        console.warn(`[TTS] voice fetch failed for ${eng.name}: HTTP ${resp.status}`);
                        return { name: eng.name, voices: [] };
                    }
                    const data = await resp.json();
                    return { name: eng.name, voices: data.voices || [] };
                } catch (e) {
                    console.warn(`[TTS] voice fetch failed for ${eng.name}: ${e?.message || e}`);
                    return { name: eng.name, voices: [] };
                }
            }));
            ...
            this._updateEngineSelects();
            this._updateModelSelect('A');
            if (this.voiceCount === 2) this._updateModelSelect('B');
            this._updateVoiceSelect('A');
            if (this.voiceCount === 2) this._updateVoiceSelect('B');
            this._voicesRecovered();
            ...
            if (this.endpoint) {
                this._voicesDown();
                this._showStatus('TTS unavailable');
            }
        } finally {
            clearTimeout(timer);
            this._voicesAbort = null;
    ...
    // ============================================
    // Catalog self-heal (issue #31, TTS half)
    // ============================================

    // Failure transition: log once, arm the fixed-interval retry. The next
    // successful loadVoices clears both (see _voicesRecovered).
    _voicesDown() {
        if (!this._voicesWasDown) {
            this._voicesWasDown = true;
            console.warn(`[TTS] nSpeech unreachable — retrying every ${VOICES_RETRY_MS / 1000}s`);
        }
        this._scheduleVoicesRetry();
    }

    // Recovery transition: log once, stop the retry loop.
    _voicesRecovered() {
        if (this._voicesRetryTimer) {
            clearTimeout(this._voicesRetryTimer);
            this._voicesRetryTimer = null;
        }
        if (this._voicesWasDown) {
            this._voicesWasDown = false;
            console.info('[TTS] nSpeech recovered — voices reloaded');
        }
    }

    _scheduleVoicesRetry() {
        if (this._voicesRetryTimer) return;
        this._voicesRetryTimer = setTimeout(() => {
            this._voicesRetryTimer = null;
            this.loadVoices();
        }, VOICES_RETRY_MS);
    }

    // Immediate refresh for user-facing failure paths (speak with no voice,
    // player error) — debounced so a failing engine can't stampede the
    // service. On failure this re-arms the retry loop via _voicesDown.
    _maybeRefreshCatalog() {
        if (!this.endpoint) return;
        if (this._voicesAbort) return; // a load is already in flight
        if (Date.now() - this._voicesLastAttempt < VOICES_REFRESH_MIN_MS) return;
        this.loadVoices();
    }
    ...
    _buildEngineOptions() {
        ...
        // Resident local engines (gpu:false) — usable without switching,
        // like cloud providers. Voices are mapped by name in loadVoices.
        // Listed before cloud engines.
        const residentEngines = this.engines
            .filter(e => !e.type && !e.gpu && e.venv_exists && e.name !== this._currentEngine)
            .sort((a, b) => (a.name || '').localeCompare(b.name || ''));
        for (const eng of residentEngines) {
    ...
    _updateEngineSelects() {
        ...
        const selects = this.voiceCount === 2
            ? [
                { el: this.elements.engineASelect, value: this.engineA, slot: 'A' },
                { el: this.elements.engineBSelect, value: this.engineB, slot: 'B' },
            ]
            : [
                { el: this.elements.engineSelect, value: this.engineA, slot: 'A' },
            ];

        for (const { el, value, slot } of selects) {
            if (!el) continue;
            ...
            // Align state + pref with the select when the stored engine is
            // genuinely invalid (catalog healthy — the engine really is gone).
            // During an outage (empty catalog) the stored engine stays
            // untouched: the mismatch is transient and self-heals on recovery
            // (issue #31). This is the old UI/state split — the select showed
            // the fallback while this.engineA kept the stale engine, so
            // resolveVoice found no voices and generation failed until the
            // user re-selected.
            if (!valid && this.engines.length > 0) {
                if (slot === 'B') {
                    this.engineB = target;
                    this.storage.setPref(PREF_ENGINE_B, target).catch(() => {});
                } else {
                    this.engineA = target;
                    this.storage.setPref(this.voiceCount === 2 ? PREF_ENGINE_A : PREF_ENGINE, target).catch(() => {});
                }
            }
    ...
    /**
     * Persist a slot's cloud sub-model slug (a concrete catalog slug).
     */
    _setSlotModel(slot, value) {
        if (slot === 'B') {
            this.modelB = value;
            this.storage.setPref(PREF_MODEL_B, value).catch(() => {});
        } else {
            this.modelA = value;
            this.storage.setPref(this.voiceCount === 2 ? PREF_MODEL_A : PREF_MODEL, value).catch(() => {});
        }
    }

    /**
     * Populate a slot's model select from the cloud model catalog. The row is
     * hidden unless the selected engine exposes more than one addressable
     * model — local engines and the 'nspeech' sentinel are dashboard-owned.
     * The provider's default model (catalog `default: true`) is always the
     * select's value unless the user picked a specific slug — the concrete
     * slug is what gets sent, no abstract "provider default" option.
     */
    _updateModelSelect(slot) {
        const selectEl = slot === 'B'
            ? this.elements.modelBSelect
            : (this.voiceCount === 2 ? this.elements.modelASelect : this.elements.modelSelect);
        if (!selectEl) return;

        const engine = slot === 'B' ? this.engineB : this.engineA;
        const models = this.modelsByEngine.get(engine) || [];

        const wrapper = selectEl.closest('nui-input-group') || selectEl.closest('.sidebar-section') || selectEl;
        wrapper.style.display = models.length > 1 ? '' : 'none';
        ...
        const stored = slot === 'B' ? this.modelB : this.modelA;
        const valid = items.some(i => i.value === stored);
        const target = valid ? stored : (models.find(m => m.default) || models[0]).id;
        if (!valid && stored) console.warn(`[TTS] stored model "${stored}" not in catalog for engine "${engine}" — using ${target}`);
        this._setSlotModel(slot, target);
        if (selectEl.setValue) selectEl.setValue(target);
    }
     ...
     * Resolve the engine + voice_id to send to nSpeech for a given slot.
     * The engine comes from the engine select (this.engineA/engineB),
     * adjusted by the cloud sub-model catalog (_resolveModel).
     * The voice comes from the voice select. Catalog-aware: when the engine's
     * voice list is known, an invalid/empty stored voice resolves to the
     * first available voice — never 'default' (kokoro's worker 500s on it).
     * Returns voice:'' only when the engine has no known voices at all —
     * callers must not send such a request.
     * @param {'A'|'B'} slot  which voice to use (ignored when voiceCount=1)
     */
    resolveVoice(slot = 'A') {
        ...
        let voice = voiceId || value || '';

        const voices = this.voicesByEngine.get(engine) || [];
        if (voices.length > 0) {
            const isValid = voices.some(v => (v.voice_id || v.name || String(v)) === voice);
            if (!isValid) voice = voiceValue(voices[0]);
        }
        return {
            model: this._resolveModel(engine, slot),
            voice,
        };
    }

    /**
     * The `model` string for the speech request: the stored cloud sub-model
     * slug when valid for the engine (nSpeech routes slugs to the specific
     * provider model), else the bare engine name (provider default).
     */
    _resolveModel(engine, slot) {
        const slug = slot === 'B' ? this.modelB : this.modelA;
        const models = this.modelsByEngine.get(engine) || [];
        if (slug && models.length > 1) {
            if (models.some(m => m.id === slug)) return slug;
            console.warn(`[TTS] stored model "${slug}" not in catalog for engine "${engine}" — using engine default`);
        }
        return engine || this.defaultModel;
    }
    ...
    _wirePlayerEvents(player) {
        ...
        player.on('error', ({ error }) => {
            ...
            // nSpeech may have bounced after a healthy catalog load — refresh
            // the catalog so recovery doesn't wait for the retry timer.
            this._maybeRefreshCatalog();
        });
        player.on('download-complete', (d) => this._emit('download-complete', d));
    }
    ...
    speak(text, targetEl, opts = {}) {
        ...
        // No voice to send — likely a failed per-engine voice fetch (see
        // loadVoices). Sending 'default' would 500 on engines like kokoro.
        if (!resolved.voice) {
            this._showStatus(`No voices available for ${resolved.model}`);
            this._maybeRefreshCatalog(); // user is actively trying — refresh now if debounce allows
            return;
        }
    ...
    /** Blob object URL of the fully generated audio (null until a generation completes). */
    getAudioUrl() { return this._speechPlayer?.getAudioUrl() ?? null; }
    ...
    /** The live <audio> element from the SDK player (null when idle). tts-player.js gates scrubbing on this. */
    get audio() { return this._speechPlayer?.audio ?? null; }
    ...
    _applyButtonState(targetEl, state) {
        ...
        if (state === 'loading') {
            ...
            if (icon) icon.setAttribute('name', 'headphones');
        }
    }
}
